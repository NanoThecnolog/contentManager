...
export class StreamService {
  ...
  private async getBucketId(): Promise<string> {
    ...
    if (!bucketName) throw new Error('Variável de ambiente B2_BUCKET_NAME não configurada.');

    await this.authorize();

    const response = await this.b2.getBucket({ bucketName });
  ...
  private async signPath(filePath: string): Promise<SignedUrl> {
    ...
    const bucketName = this.configService.get<string>('B2_BUCKET_NAME') ?? '';
    const ttlSeconds = Number(this.configService.get<string>('B2_URL_TTL_SECONDS')) || 3600;
    ...
    try {
      ...
      throw new ServiceUnavailableException('Não foi possível assinar a URL do conteúdo.');
    }
  }
  ...
  async signEpisode(tmdbId: number, season: number, episode: number): Promise<SignedUrl> {
